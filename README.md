# golang-tpt
golang test programs

# references

[The Go Memory Model](https://go.dev/ref/mem)

[Go Concurrency Patterns](https://go.dev/talks/2012/concurrency.slide#1)

[A Guide to the Go Garbage Collector](https://tip.golang.org/doc/gc-guide)

[Strings, bytes, runes and characters in Go](https://go.dev/blog/strings)

[Go Slices: usage and internals](https://go.dev/blog/slices-intro)

[Go maps in action](https://go.dev/blog/maps)

[Range Over Function Types - The Go Programming Language](https://go.dev/blog/range-functions)

[Go Wiki: Common Mistakes - The Go Programming Language](https://go.dev/wiki/CommonMistakes)

[go compiler intro](https://go.dev/src/cmd/compile/README)

[Effective Go](https://go.dev/doc/effective_go)

[A Tour of Go](https://go.dev/tour/list)

[Go blog](https://go.dev/blog/all)

[go builtins](https://pkg.go.dev/builtin)

[go generics](https://go.dev/doc/tutorial/generics)

[Errors in Go](https://go.dev/blog/go1.13-errors)

[JSON and Go](https://go.dev/blog/json)

[go wiki slog](https://go.dev/wiki/Resources-for-slog)

[Go Wiki: Home - The Go Programming Language](https://go.dev/wiki/)

[compiler directives](https://pkg.go.dev/cmd/compile#hdr-Compiler_Directives)

[Go's Concurrency Model - Syntio](https://www.syntio.net/en/labs-musings/gos-concurrency-model/)

[Go Interfaces: under the hood](https://research.swtch.com/interfaces)

[go generate guide](https://eli.thegreenplace.net/2021/a-comprehensive-guide-to-go-generate)

[vim-go Wiki](https://github.com/fatih/vim-go/wiki/Tutorial#quick-setup)

[Go by Example](https://gobyexample.com/)

[Language Mechanics On Stacks And Pointers](https://www.ardanlabs.com/blog/2017/05/language-mechanics-on-stacks-and-pointers.html)

[High Performance Go Workshop](https://dave.cheney.net/high-performance-go-workshop/dotgo-paris.html?source=post_page-----4b79d4f6cd65---------------------------------------#watch_out_for_compiler_optimisations)

[Understanding the context package in golang - Parikshit Agnihotry](https://p.agnihotry.com/post/understanding_the_context_package_in_golang/?source=post_page-----4b79d4f6cd65---------------------------------------)

[gRPC and Deadlines | gRPC](https://grpc.io/blog/deadlines/?source=post_page-----4b79d4f6cd65---------------------------------------)

[Does the Go race detector catch all data race bugs? | by Val Deleplace | Medium](https://medium.com/@val_deleplace/does-the-race-detector-catch-all-data-races-1afed51d57fb)

[The Top 10 Most Common Mistakes I’ve Seen in Go Projects | by Teiva Harsanyi | ITNEXT](https://itnext.io/the-top-10-most-common-mistakes-ive-seen-in-go-projects-4b79d4f6cd65)

[Go init Function](https://tutorialedge.net/golang/the-go-init-function/)
